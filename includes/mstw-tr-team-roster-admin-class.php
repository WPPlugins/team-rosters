 ...
 *	Copyright 2017 Mark O'Donnell (mark@shoalsummitsolutions.com)
 *
...
class MSTW_TR_TEAM_ROSTERS_ADMIN {
	...
	function __construct( ) {
		//mstw_tr_log_msg( " MSTW_TR_TEAM_ROSTERS_ADMIN.__construct" );
	...
	function add_players_screen( ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.add_players_screen:" );
	...
	function edit_roster_screen(  ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.edit_roster_screen:" );
	...
	function screen_builder( $edit = 1 ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.screen_builder:" );
	...
	function build_roster_table_header( ) {
		//mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN.build_roster_table_header:' );
		
		$data_fields = self::$data_fields;
		
		$options = wp_parse_args( get_option( 'mstw_tr_options' ), mstw_tr_get_defaults( ) );
		
		//mstw_tr_log_msg( 'data_fields' );
		//mstw_tr_log_msg( $data_fields );
		?>
	...
	function build_roster_table_body( $players ) {
		//mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN:build_roster_table_body:' );
		...
		if ( $players -> have_posts( ) ) : 
			while ( $players -> have_posts( ) ) : 
				...
				//mstw_tr_log_msg( "current title: " . get_the_title( ) );
				//mstw_tr_log_msg( "current ID: " . get_the_ID( ) );
				
				$player = get_post( get_the_ID( ) );
				
				//mstw_tr_log_msg( $player );
	...
	function build_add_players_table_body( ) {
		//mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN:build_add_players_table_body:' );
	...
	function build_table_row( $player =  null, $row_nbr = 0 ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN:build_table_row: " );
		...
		if ( null === $player ) {
			//mstw_tr_log_msg( "Building a blank row" );
			?>
	...
	function build_text_cell( $value = '', $tag = 'tr_text', $size = 16, $maxlength = 32 ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN:build_text_cell: " );
	...
	function build_pagination_links( $page, $max_num_pages ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN:build_pagination_links: " );
	...
	function make_tag( $base, $suffix ) {
		//mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN:make_tag' );
	...
	function get_tag_base( $tag, $separator = "_" ) {
		//mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN:get_tag_base' );
		//mstw_tr_log_msg( "tag: $tag" );
		//mstw_tr_log_msg( "separator: $separator" );
		
		$us_pos = strrpos( $tag, $separator );
		//mstw_tr_log_msg( "us_pos: $us_pos" );
	...
	function build_team_select( $current_team, $css_tag = '', $edit = 1 ) {
		//mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN:build_team_select' );
	...
	function build_team_list( ) {
		//mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN:build_team_list:' );
		...
		if ( $teams ) {
			...
			//mstw_tr_log_msg( 'build_team_list: no teams found' );
			return '';
	...
	function post( $options ) {
		//mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN.post:' );
		//mstw_tr_log_msg( $options );
		//mstw_tr_log_msg( $_POST );
		
		if ( !$options ) {
			mstw_tr_add_admin_notice( 'error', __( 'Problem encountered updating games. Exiting.', 'team-rosters' ) );
			mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN.post: Problem encountered updating games. Exiting.' );
			return;
		}
		...
		switch( $options['submit_value'] ) {
			...
			case __( 'Add Players', 'team-rosters' ):
				//mstw_tr_log_msg( 'Updating Roster ...' );

				//if ( true ) { // for testing
				if ( !array_key_exists( 'current-team', $_POST ) ) {
					mstw_tr_log_msg( 'MSTW_TR_TEAM_ROSTERS_ADMIN.post: No team specified for roster update? ' );
					...
					while ( $elt = each ( $_POST ) ) {
						//mstw_tr_log_msg( "elt: " );
						//mstw_tr_log_msg( $elt );
						
						if ( 'player_slug' == $this -> get_tag_base( $elt['key'] ) ) {
							//mstw_tr_log_msg( "Found possible player data set " );
							//mstw_tr_log_msg( "player_slug: " . $elt['value'] );
							
							$player_slug = $this -> get_tag_base( $elt['value'] );
							//mstw_tr_log_msg( "player_slug: $player_slug" );
							
							$result = $this -> process_player( $player_slug, $team_slug );
							
							//mstw_tr_log_msg( "process_player returned: $result" );
							...
							$result = -1;
							mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.post: No player slug found. Skipping." );
					...
					//
					//mstw_tr_log_msg( "$nbr_updated players updated" );
					if ( $nbr_updated ) {
						mstw_tr_add_admin_notice( 'updated', "$nbr_updated players updated." );
					}
					
					//mstw_tr_log_msg( "$nbr_created players created" );
					if ( $nbr_created ) {
			...
			default:
				...
				mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.post: received unknown " . $options['submit_value'] );
				break;
	...
	//	  Player data is in the $_POST auto global
	//
	//	RETURNS:
	//	  None. Creates the player CPT if necessary & updates the metadata
	//
	function process_player( $player_slug, $team_slug ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.process_player:" );
		//mstw_tr_log_msg( "player_slug: $player_slug" );
		...
		$first = trim( each( $_POST )['value'] );
		$last  = trim( each( $_POST )['value'] );
		...
		if ( -1 ==  $player_slug ) {
			...
			if ( $player_title ) {
				...
				if ( $player_id ) {
					$player_slug = '';
					$retval = $this -> update_player( $player_id, $player_slug, $team_slug, $first, $last );
				...
				//mstw_tr_log_msg( "No first or last. We are done!" );
				$retval = -1;
			}
		
		} else if ( $player_slug ) {
			//mstw_tr_log_msg( "Updating player: $player_slug" );
			$player_id = 0;
			$retval = $this -> update_player( $player_id, $player_slug, $team_slug, $first, $last );
			...
			// mstw_tr_log_msg( $this -> get_tag_base( $elt['key'] ) );
	...
	function create_player( $player_title, $team_slug ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.create_player:" );
	...
	//	  $team_slug:   team (taxonomy) slug
	//	  $first_name:  player first name (already pulled from $_POST[])
	//	  $last_name:   player last name (already pulled from $_POST[])
	//
	//	RETURNS:
	//	  1 on success; 0 on failure
	//
	function update_player( $player_id, $player_slug, $team_slug, $first_name, $last_name ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.update_player:" );
		
		// $_POST is an auto global
		
		//mstw_tr_log_msg( "Updating player id: $player_id and slug: $player_slug for team $team_slug" );
		//mstw_tr_log_msg( $_POST );
		...
		if ( '' != $player_slug ) {
			...
			//if( true ) { //for testing
			if ( $player_obj == null ) {
				mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.update_player: Player update failed: $first_name $last_name" );
		...
		if ( 0 != $player_id ) {
			...
			//mstw_tr_log_msg( "player: $player_slug ID: $player_id" );
		
			foreach ( self::$data_fields as $key => $value ) {
				//mstw_tr_log_msg( $key );
				if ( 'player_first_name' == $key ) {
					update_post_meta( $player_id, "player_first_name", $first_name );
					
				} else if ( 'player_last_name' == $key ) {
					update_post_meta( $player_id, "player_last_name", $last_name );
					
				} else {
					$value = each( $_POST )['value'];
	...
	//	
	function add_help( ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.add_help:" );
	...
	function edit_rosters_help_content( ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.edit_rosters_help_content:" );
	...
	function add_players_help_content( ) {
		//mstw_tr_log_msg( "MSTW_TR_TEAM_ROSTERS_ADMIN.add_players_help_content:" );
