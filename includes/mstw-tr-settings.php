...
if( !function_exists( 'mstw_tr_settings_page' ) ) {
	function mstw_tr_settings_page( ) {
		//mstw_tr_log_msg( "mstw_tr_settings_page:" );
...
if( !function_exists( 'mstw_tr_admin_tabs' ) ) {
	function mstw_tr_admin_tabs( $current_tab = 'data-fields-columns-tab' ) {
		$tabs = array( 	'data-fields-columns-tab' => __( 'Data Fields & Columns', 'team-rosters' ),
...
if( !function_exists( 'mstw_tr_settings_help' ) ) {
	function mstw_tr_settings_help( ) {
		//mstw_tr_log_msg( "mstw_tr_settings_help:" );
...
if( !function_exists( 'mstw_tr_data_fields_columns_help' ) ) { 
	function mstw_tr_data_fields_columns_help( ) {
		$help = '<h3><strong>' . __( 'Data Fields & Columns Settings:', 'team-rosters' ) . '</strong></h3>' .
				'<p>' . __('This screen controls the visibility of data fields, the corresponding columns, and the field/column labels, for the roster front end displays. ', 'team-rosters' ) . "</p>\n" .
...
if( !function_exists( 'mstw_tr_roster_tables_help' ) ) { 
	function mstw_tr_roster_tables_help( ) {
		$help = '<h3><strong>' . __( 'Roster Tables Settings:', 'team-rosters' ) . '</strong></h3>' .
				'<p>' . __('This screen provides controls for a number of fields in the roster tables, related primarily to data formats.  Note that these settings apply to ALL roster tables on the site.', 'team-rosters' ) . "</p>\n" .
				'<p>' . __('Note that these settings apply to ALL roster tables on the site. To control individual tables by team, set the corresponding arguments in the shortcodes.', 'team-rosters' ) . "</p>\n" .
...
if( !function_exists( 'mstw_tr_roster_table_colors_help' ) ) { 	
	function mstw_tr_roster_table_colors_help( ) {
		$help = '<h3><strong>' . __( 'Roster Table Color Settings:', 'team-rosters' ) . '</strong></h3>' .
				'<p>' . sprintf( __('This screen controls the colors for the roster tables. Note that these settings apply to ALL roster tables on a site. It is recommended that you use custom stylesheets (CSS) to control the colors of all front end displays. %sSee the Team Rosters users manual for more information on customization by team%s.', 'team-rosters' ), '<a href="http://shoalsummitsolutions.com/tr-customizing/" target="_blank">', '</a>' ) . "</p>\n" .
...
if( !function_exists( 'mstw_tr_validate_settings' ) ) { 
	function mstw_tr_validate_settings( $input ) {
		//mstw_tr_log_msg( 'mstw_tr_validate_settings:' );
		//mstw_tr_log_msg( $_POST );
		//mstw_tr_log_msg( '$input = ' );
		//mstw_tr_log_msg( $input );
		...
		if ( array_key_exists( 'reset', $input ) ) {
			//mstw_tr_log_msg( 'OK, we are looking to reset defaults $current_tab = ' . $current_tab );
			if( $input['reset'] == 'Resetting Defaults' ) {
				...
				switch( $current_tab ) {
					case 'data-fields-columns-tab':
						...
						$msg = __( 'Data fields & columns settings reset to defaults.', 'team-rosters');
						break;
					case 'roster-table-tab':
			...
			switch ( $current_tab ) {
				case 'data-fields-columns-tab':
					//mstw_tr_log_msg( 'validating ... $current_tab= data-fields-columns-tab' );
				...
				case 'roster-colors-tab':
					//mstw_tr_log_msg( 'validating ... $current_tab= roster-colors-tab' );
				...
				case 'bio-gallery-tab':
					//mstw_tr_log_msg( 'validating ... $current_tab= bio-gallery-tab' );
					// checkboxes are unique
