...
//		
	...
	if( !function_exists( 'mstw_tr_bio_gallery_section_setup' ) ) {
		function mstw_tr_bio_gallery_section_setup( ) {		
			...
			//mstw_tr_log_msg( 'in mstw_tr_data_fields_section_setup ...' );
			//mstw_tr_log_msg( '$display_on_page= ' . $display_on_page );
			//mstw_tr_log_msg( '$page_section= ' . $page_section );
			//mstw_tr_log_msg( '$options= ' );
			//mstw_tr_log_msg( $options );
			...
			$arguments = array(
				...
				array( 	// PLAYER PHOTO WIDTH
					...
					'desc'	=> __( 'In pixels. (Defaults to blank, which means the stylesheet setting will be used; 150px out of the box.)', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
