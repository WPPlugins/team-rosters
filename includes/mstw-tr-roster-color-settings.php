 ...
 *--------------------------------------------------------------------------*/
	...
	if( !function_exists( 'mstw_tr_roster_colors_setup' ) ) {
		function mstw_tr_roster_colors_setup( ) {
			//mstw_tr_log_msg( 'in mstw_tr_roster_colors_setup ...' );
	...
	if( !function_exists( 'mstw_tr_table_colors_section_setup' ) ) {
		function mstw_tr_table_colors_section_setup( ) {		
			...
			add_settings_section(
				$page_section,
				__( 'Table Color Settings', 'team-rosters' ),
			...
			$arguments = array( 
				array( 	// USE TEAM COLORS
					...
					'value' => mstw_tr_safe_ref( $options, 'use_team_colors' ), 
					'title' => __( 'Use Team Colors:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
					'desc' => __( 'Use the colors from the Teams table in MSTW Schedules & Scoreboards. IGNORED if the S&S Teams DB (v4.0+)is not found. Note: There is a separate setting for player profiles and galleries.', 'team-rosters' ),
				),
				array( 	// ROSTER TITLE COLOR
					...
					'title' => __( 'Roster Table Title:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
				array( 	// ROSTER TABLE LINKS COLOR
					...
					'title' => __( 'Roster Table Links:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
				array( 	// TABLE HEADER BACKGROUND
					...
					'title' => __( 'Table Header Background:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
				array( 	// TABLE HEADER TEXT
					...
					'title' => __( 'Table Header Text:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
				array( 	// EVEN ROW BACKGROUND
					...
					'title' => __( 'Even Row Background:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
				array( 	// EVEN ROW TEXT
					...
					'title' => __( 'Even Row Text:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
				array( 	// ODD ROW BACKGROUND
					...
					'title' => __( 'Odd Row Background:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
				array( 	// ODD ROW TEXT
					...
					'title' => __( 'Odd Row Text:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
				array( 	// TABLE BORDERS
					...
					'title' => __( 'Table Borders:', 'team-rosters' ),
					'page' => $display_on_page,
					'section' => $page_section,
				),
			);
	...
	if( !function_exists( 'mstw_tr_table_color_inst' ) ) {
		function mstw_tr_table_color_inst( ) {
			echo '<p>' . __( 'Note that these settings will apply to ALL the [shortcode] roster tables, overriding the default styles. However they can be overridden by more specific stylesheet rules for specific teams. See the plugin documentation for more details.', 'team-rosters' ) . '</p>';
